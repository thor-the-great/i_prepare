...
public:
    //Idea do BFS in graph where verteces are employees and managers and edges are times.
    //do the BFS and save current max edge length 
    int numOfMinutes(int n, int headID, vector<int>& manager, vector<int>& informTime) {
        ...
        vector<vector<int>> managersMap(n);
        for(int i = 0; i < n; i ++) {   
            if(i == headID)
                continue;
            managersMap[manager[i]].push_back(i);
        }
        ...
        while(!q.empty()) {
            ...
            for (int i = 0; i < size; i++) {
                ...
                for(auto s : managersMap[cur]) {
                    q.push(s);
                    times[s] = times[cur] + informTime[cur];
                    res = max(res, times[s]);
                }
            }
        }
