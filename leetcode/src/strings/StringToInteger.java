...
public class StringToInteger {
    ...
    public int myAtoi(String str) {
        ...
        for (char ch : str.toCharArray()) {
            ...
            if (ch == ' ' && !started)
                continue;
            //check for fist number or sign char
            if (!started && (ch == '+' || ch == '-' || (ch >= '0' && ch <= '9'))) {
                started = true;
                if ((ch >= '0' && ch <= '9')) {
                    cur = cur * 10 + (ch - '0');
                } else if (ch == '-') {
                ...
                //parse next number, check if we got our of range
                cur = cur * 10 + (ch - '0');
                if (negative && -cur < Integer.MIN_VALUE)
                    return Integer.MIN_VALUE;
                else if (!negative && cur > Integer.MAX_VALUE)
                    return Integer.MAX_VALUE;
            } else {
                //next non-number after we started means we need to break out of loop
                break;
            }
        }
        return negative ? (int) -cur : (int) cur;
    }
}
