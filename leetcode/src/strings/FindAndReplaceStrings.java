 ...
 *
 * For example, if we have S = "abcd" and we have some replacement operation i = 2, x = "cd", y =
 * "ffff", then because "cd" starts at position 2 in the original string S, we will replace it
 * with "ffff".
 *
 * Using another example on S = "abcd", if we have both the replacement operation i = 0, x =
 * "ab", y = "eee", as well as another replacement operation i = 2, x = "ec", y = "ffff", this
 * second operation does nothing because in the original string S[2] = 'c', which doesn't match
 * x[0] = 'e'.
 *
 * All these operations occur simultaneously.  It's guaranteed that there won't be any overlap in
 * replacement: for example, S = "abc", indexes = [0, 1], sources = ["ab","bc"] is not a valid
 * test case.
 *
 * Example 1:
 *
...
public class FindAndReplaceStrings {
    ...
    /**
     * Create a map of indexes, then while going over the string check if this index is in the map
     * and if yes- try to perform replacement
     * @param S
     * @param indexes
     * @param sources
     * @param targets
    ...
    public String findReplaceString(String S, int[] indexes, String[] sources, String[] targets) {
        ...
        while (i< S.length()) {
            if (!m.containsKey(i))
                ...
                String[] replacement = m.get(i);
                if (S.startsWith(replacement[0], i)) {
