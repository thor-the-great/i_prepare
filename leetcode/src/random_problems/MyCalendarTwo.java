...
public class MyCalendarTwo {
  ...
  public boolean book(int start, int end) {
    ...
    for (int[] twos : overlaps) {
      if (end > twos[0] && start < twos[1])
        return false;
    }


    for (int[] ones : intervals) {
      if (start < ones[1] && end > ones[0]) {
        overlaps.add(new int[] {Math.max(ones[0], start), Math.min(ones[1], end)});
      }
    }
