...
public class DistanceBetweenBusStops {
    ...
    public int distanceBetweenBusStops(int[] distance, int start, int destination) {
        int sum1 = 0, sum2 = 0;
        //make destination always greater equals
        if (start > destination) {
            int t = start;
            start = destination;
            destination = t;
        }
        //iterate over every distnace
        for (int i = 0; i < distance.length; i++) {
            //if this stop is in circle 1 - add it to sum 1, otherwise - add to sum 2
            if (i >= start && i < destination)
                sum1+=distance[i];
            else
                sum2+=distance[i];
        }
        //choose minimal of two
