 ...
 * You may assume each given directory info represents a unique directory. Directory path and file info are separated
 ...
 */
public class DuplicateFileInFilesystems {
    /**
     * Idea - hold map with content as key and matched paths as values (list). Iterate and fill the map, then
     * put to result those values where size >= 2
     * @param paths
     * @return
     */
    public List<List<String>> findDuplicate(String[] paths) {
        ...
        for (String path : paths) {
            ...
            for (int i = 1; i < pathParts.length; i++) {
                int idxOfOpen = pathParts[i].indexOf("(");
                String fileName = pathParts[i].substring(0, idxOfOpen);
                String cont = pathParts[i].substring(idxOfOpen + 1, pathParts[i].length() - 1);
                String filePath = dir + "/" + fileName;
                if (!m.containsKey(cont)) {
                    m.put(cont, new ArrayList());
                }
                m.get(cont).add(filePath);
            }
        }

        for (String cont : m.keySet()) {
            if (m.get(cont).size() >= 2)
                res.add(m.get(cont));
        }
        return res;
    }
