...
public class AutocompleteSystem {
    ...
    Comparator<SearchObject> comp = (o1, o2) -> {
        if (o1.rank != o2.rank)
    ...
    void addSentenceToTrie(String s, int r) {
        Trie n = t;
        for (char ch : s.toCharArray()) {
            int idx = ch == ' ' ? 26 : ch - 'a';
            if (n.next[idx] == null)
                n.next[idx] = new Trie();
            n.next[idx].insertSentence(s, r);
            n = n.next[idx];
        }
    }
    ...
    public List<String> input(char c) {
        ...
        if (c == '#') {
            String newSentence = query.toString();
            addSentenceToTrie(newSentence, 1);
            query.setLength(0);
            sTrie = t;
            ...
            if (sTrie == null)
                return new ArrayList();
            int idx = c == ' ' ? 26 : c - 'a';
            sTrie = sTrie.next[idx];
            if (sTrie == null) {
                return new ArrayList();
            }
            if (!sTrie.sorted) {
                Collections.sort(sTrie.list, comp);
                sTrie.sorted = true;
            }
            List<String> res = new ArrayList();
            for (int i =0; i < Math.min(3, sTrie.list.size()); i++) {
                res.add(sTrie.list.get(i).sentence);
            }
            return res;
        }
    }
...
class Trie {
    ...
    Trie(String sentence, int rank) {
        this();
        insertSentence(sentence, rank);
    }

    void insertSentence(String sentence, int rank) {
        sorted = false;
        boolean found = false;
        for (int i = 0; i < list.size(); i++) {
            SearchObject obj = list.get(i);
            if (obj.sentence.equals(sentence)) {
                obj.rank += rank;
                found = true;
                break;
            }
        ...
        if (!found) {
            SearchObject s = new SearchObject(sentence, rank);
            list.add(s);
        }
    }
}
...
class SearchObject {
    ...
    SearchObject(String sentence, int rank) {
