...
public class PourWater {
    ...
    /**
     * Idea - start from moving droplet to the left. When done - check if it's possible to move it to the right (cases
     * when there is lower level several cells long but it doesnt end with the hole. Then eventually move droplet again
     * to the left, but to the K cell (so it tops at K max)
     * @param heights
     * @param V
     * @param K
     * @return
    ...
    public int[] pourWater(int[] heights, int V, int K) {
        ...
        while (V-- > 0) {
            ...
            while (c < N - 1 && heights[c + 1] <= heights[c])
                c++;
    ...
    public static void main(String[] args) {
        ...
        int[] res = obj.pourWater(new int[] {1,2,3,4,3,2,1,2,3,4,3,2,1}, 5, 5);
        System.out.println(StringUtils.intArrayToString(res));

        res = obj.pourWater(new int[] {1,2,3,4}, 2, 2);
        System.out.println(StringUtils.intArrayToString(res));
    }
}
