...
public class TopVotedCandidate {
  ...
  TreeMap<Integer, Integer> votesByTime;

  public TopVotedCandidate(int[] persons, int[] times) {
    int max = -1, winner = 0;
    int[] votes = new int[persons.length];
    votesByTime = new TreeMap();

    for (int i = 0; i < persons.length; i++) {
      ++votes[persons[i]];
      int curVotes = votes[persons[i]];
      if (curVotes >= max) {
        max = curVotes;
        winner = persons[i];
      }
      votesByTime.put(times[i], winner);
    }
  }

  public int q(int t) {
    return votesByTime.get(votesByTime.floorKey(t));
  }
}
