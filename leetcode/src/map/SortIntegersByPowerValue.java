 ...
 * which is 13.
 * Notice that 12 and 13 have the same power value and we sorted them in ascending order. Same
 * for 14 and 15.
 * Example 2:
 *
 * Input: lo = 1, hi = 1, k = 1
...
public class SortIntegersByPowerValue {
  ...
  public int getKth(int lo, int hi, int k) {
    //init our map
    map.put(1, 0);
    //form the max heap with custom comparator as per problem statement
    PriorityQueue<Integer> pq = new PriorityQueue<>(
        (i1, i2) -> (getPower(i1) != getPower(i2))
            ? getPower(i2) - getPower(i1)
            : i2 - i1
    );
    //start filling max heap with values
    for (int i = lo; i <= hi; i++) {
      pq.add(i);
      //if have more that k then last one (with most power value) must be popped out
      if (pq.size() > k)
        pq.poll();
    }
    return pq.peek();
  ...
  int getPower(int i) {
    //first check the map if we have met thi value before
    if (map.containsKey(i))
      return map.get(i);
    //do recursive call and add 1 for the current number
    int res = 1 + ((i%2 == 0) ? getPower(i/2) : getPower(3*i + 1));
    //save to cache map before returning result
    map.put(i, res);
    return res;
  }
}
