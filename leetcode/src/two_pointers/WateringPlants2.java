...
public class WateringPlants2 {
    /**
     * Simulate process with 2 pointers going to center from both ends. Middle plant is simulated as a special case
     * 
     * O(n) time
     * O(1) space
     */
    public int minimumRefill(int[] plants, int capacityA, int capacityB) {
        ...
        if (plants.length % 2 == 1) {
            int middlePlant = plants[plants.length/2];
            if (middlePlant > Math.max(amountA, amountB)) {
                refills++;
            }    
        }
        return refills;
