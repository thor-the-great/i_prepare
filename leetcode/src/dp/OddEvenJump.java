...
public class OddEvenJump {
    ...
    /**
     * Look to the problem from the last to 0 index. For last one index solution is possible. For next one we can get
     * proper index to jump based on prev value and it's state. If the same value we've seen already - this is our jump
     * otherwise look for lowest or highest key respectively. Result of jumps keep in two arrays - one for even one for
     * odd indexes. For every jump
     * odd[i] = even[map.getSameOrHigher(i)];
     * even[i] = odd[map.getSameOrLower(i)];
     * At the end count number of true's in odd jump array - we start from 0, then for 1 - we've started from even[0]
     * but must jump to odd[1], so odd[1] must be set.
     * Count how many odd[i] == true - this is our result
     * @param A
     * @return
     */
    public int oddEvenJumps(int[] A) {
        ...
        for (int i = N - 2; i>= 0; i--) {
            if (map.containsKey(A[i])) {
                int next = map.get(A[i]);
                odd[i] = even[next];
                even[i] = odd[next];
                if (odd[i])
                    ++res;
            } else {
                Integer h = map.higherKey(A[i]);
                if (h != null) {
