...
public class TimeMap {
  ...
  /**
   * Create map of bst, in bst we store timestamp as node and value as a value, we can search
   * then in O(lgn). For key mapping use normal hashmap
   */
  public TimeMap() {
    m = new HashMap();
  }
  ...
  public String get(String key, int timestamp) {
    ...
    TreeMap<Integer, String> keyTreeMap = m.get(key);
    Integer lessOrEqTimestamp = keyTreeMap.floorKey(timestamp);
    if (lessOrEqTimestamp == null)
      return "";
    return keyTreeMap.get(lessOrEqTimestamp);
  }
}
