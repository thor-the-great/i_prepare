...
public class TimePlanner {
     ...
     * duration dur, returns the earliest time slot that works for both of them and is of duration dur. If there is
     * no common time slot that satisfies the duration requirement, return an empty array.
     *
     ...
     *
     * Each person’s availability is represented by an array of pairs. Each pair is an epoch array of size two. The
     * first epoch in a pair represents the start time of a slot. The second epoch is the end time of that slot. The
     * input variable dur is a positive integer that represents the duration of a meeting in seconds. The output is
     * also a pair represented by an epoch array of size two.
     *
    ...
    static int[] meetingPlanner(int[][] slotsA, int[][] slotsB, int dur) {
        ...
        while (pA < slotsA.length && pB < slotsB.length) {
            ...
            int maxStart = Math.max(slotA[0], slotB[0]);
            int minEnd = Math.min(slotA[1], slotB[1]);
